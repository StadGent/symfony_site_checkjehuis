...
class SubsidyCalculator
{
    ...
    /**
     * @param array|\Digip\RenovationBundle\Entity\Subsidy[] $windRoofSubsidies
     * @return $this
     */
    ...
    /**
     * @param array|\Digip\RenovationBundle\Entity\Subsidy[] $solarHeaterSubsidies
     * @return $this
     */
    ...
    {
        // all roof related subsidies combined can not go over the max of $this->subsidyCeilingGentRoof
        $roofRelated = 0;
        $roofSubsidies = array(
            'category' => array(),
        ...
        foreach ($this->house->getUpgradeConfigs() as $c) {
            ...
            foreach ($c->getSubsidies() as $s) {
                ...
                // except for a 100% flat roof (no wind roof possible in that case)
                // or if you place the isolation on the floor... which is a "generic" config choice, so hardcoded on ID
        ...
        // if we have roof related stuff, check if we haven't gone over the limit
        if ($roofRelated) {
    ...
    protected function assertRoofSubsidyLimits($total, $subsidies, $roofConfigCategory)
    {
        ...
        if ($count && $total > $this->subsidyCeilingGentRoof) {
            ...
            // update the details, spread subsidies pro-rata over the active parts
