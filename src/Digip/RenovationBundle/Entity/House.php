...
class House
{
    ...
    public function setElectricHeating($electricHeating)
    {
        ...
        foreach ($this->configs as $c) if ($c->getCategory()->getSlug() == $slug)
            $this->configs->removeElement($c);
        foreach ($this->upgradeConfigs as $c) if ($c->getCategory()->getSlug() == $slug)
    ...
    public function getAllRenewables()
    {
        ...
        foreach ($this->getRenewables() as $r) $renewables[$r->getSlug()] = $r;
        foreach ($this->getUpgradeRenewables() as $r) $renewables[$r->getSlug()] = $r;
